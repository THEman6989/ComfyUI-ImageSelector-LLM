...
| `reranker_endpoint` | STRING | Optional reranker server base URL, for example `http://127.0.0.1:8012`. Supports llama.cpp, SGLang, and vLLM rerank responses. |
...
The main `endpoint` uses OpenAI-compatible chat completions. It can point at llama.cpp, SGLang, LMDeploy, vLLM, or another compatible vision server as long as it accepts image content at `/v1/chat/completions`.
...
For SGLang, LMDeploy, or vLLM, use their normal OpenAI-compatible chat endpoint, for example `http://127.0.0.1:30000/v1/chat/completions` for SGLang, `http://127.0.0.1:23333/v1/chat/completions` for LMDeploy, or `http://127.0.0.1:8000/v1/chat/completions` for vLLM.
...
Set `reranker_endpoint` to use a reranker before the final vision LLM call. The node probes `/v1/rerank`, `/v2/rerank`, `/rerank`, `/reranking`, and `/v1/reranking` with a tiny test request and only uses the reranker when it returns usable scores. It parses llama.cpp/vLLM-style JSON objects with `results` and SGLang-style top-level result lists. If `reranker_model` is empty, it tries `/v1/models` first and uses the first returned model id, which is useful for SGLang and vLLM where `/v1/rerank` normally expects `model`.

Reranking here is text-based, so it ranks folder names and candidate source paths; it does not inspect image pixels. Start a llama.cpp reranker server with a reranking GGUF model and llama.cpp's reranking options, for example `llama-server -m /path/to/reranker.gguf --embedding --pooling rank --reranking --host 127.0.0.1 --port 8012`. For SGLang cross-encoder rerankers, launch with a rerank model and call the same base URL, for example `python3 -m sglang.launch_server --model-path BAAI/bge-reranker-v2-m3 --is-embedding --host 0.0.0.0 --port 30000`. For vLLM rerankers, serve a scoring/pooling model and use the OpenAI-compatible base URL, for example `vllm serve BAAI/bge-reranker-v2-m3 --task score --host 0.0.0.0 --port 8000`.
